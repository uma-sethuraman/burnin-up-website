...
class City(db.Model):
    ...
    def __init__(self, city_name="NaN", population=0, time_zone="NaN", elevation=0, lat=0.0, long=0.0, pm25=-1.0,
                 pm10=-1.0, o3=-1.0, country_iso2code="NaN", co=-1.0):
...
for city in city_temp_table:
    exist_temp = db.session.query(db.session.query(City).filter_by(city_name=city).exists()).scalar()
    if not exist_temp:
        combined = city.split()
        if len(combined) > 1:
...
country_capitals = {item for item in db.session.query(Country.country_capital_city).all()}
...
for city in added_cities:
    req = "https://public.opendatasoft.com/api/records/1.0/search/?dataset=worldcitiespop&q=" + city +"&sort=population&facet=country"
    ...
    if (response.status_code == 200):
        ...
        if len(response["records"]):
            ...
            new_city = City(lat = record["latitude"], long=record["longitude"], city_name=record["city"].capitalize(), population=pop, country_iso2code=record["country"].upper())
            new_add += [new_city]

print("num new_add: ", len(new_add))

for city in new_add:
    req = "https://api.waqi.info/feed/geo:" + str(city.lat) + ";" + str(city.long) + "/?token=1cbf10be27bc7aa662b54f38d9c0d0a592eba24c"
    response = requests.request("GET", req)
    if (response.status_code == 200):
        ...
        if "data" in cities_climate_data:
            if "forecast" in cities_climate_data["data"]:
                ...
                if "daily" in cities_climate_data["data"]["forecast"]:
                    city.setpm25(cities_climate_data["data"]["forecast"]["daily"]["pm25"][0]["avg"])
                    city.setpm10(cities_climate_data["data"]["forecast"]["daily"]["pm10"][0]["avg"])
                    city.seto3(cities_climate_data["data"]["forecast"]["daily"]["o3"][0]["avg"])
